    ...
    return (
        <div>
            <div className="flex justify-center">
                <button onClick={handleCreateExpense} className="m-2 bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4 border-b-4 border-blue-700 hover:border-blue-500 rounded">
                    Create expense
                </button>
            </div>
                ...
                <tbody>
                    {expenses.map((exp) => (
                        <tr key={exp.id} className="border-b hover:bg-yellow-100 cursor-pointer transition-colors" onClick={() => handleRowClick(exp.id)}>
                            <td className="px-4 py-2">{exp.amount < 0 ? "-$" : "$"}{Math.abs(exp.amount)}</td>
